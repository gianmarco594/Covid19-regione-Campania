...
fs.readdir('./Dataset_Covid-19', (err, files) => {
    if (!files.length) {
        ...
        var file = 'dpc-covid19-ita-regioni-'+date_ob.getFullYear() + '' + ("0" + (date_ob.getMonth() + 1)).slice(-2) + '' + ("0" + date_ob.getDate()).slice(-2) + '.csv';
        if (Namefile == file) {
            ...
            request.get(url, function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    var csv = body;
                    var file = './Dataset_Covid-19/' + file;
                        ...
                        request.get(url, function (error, response, body) {
                            if (!error && response.statusCode == 200) {
                                ...
                                fs.writeFile(file, body, function (err) {
                                    ...
                                    console.log('done');
                                    //eliminzaione file
                                    Nomefile = './Dataset_Covid-19/' + Nomefile;
